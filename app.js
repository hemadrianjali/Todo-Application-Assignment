...
const hasCategoryAndSPriorityProperties = requestQuery => {
  return (
...
app.get('/todos/', async (request, response) => {
  ...
  switch (true) {
    ...
    case hasCategoryAndStatusProperties(request.query):
      ...
      break
    case hasCategoryAndSPriorityProperties(request.query):
      getTodosQuery = `
            SELECT * FROM todo
            WHERE
            todo LIKE '%${search_q}%'
...
app.post('/todos/', async (request, response) => {
  const todoDeatails = request.body
  const {id, todo, priority, status, category, dueDate} = todoDeatails
  switch (false) {
...
app.put('/todos/:todoId/', async (request, response) => {
  const {todoId} = request.params
  const todoDeatails = request.body
  const {todo, priority, status, category, dueDate} = todoDeatails
  switch (true) {
    ...
    case hasPriorityProperty(request.body):
      const updateTodoPriorityQuery = `UPDATE todo
      SET
      priority='${priority}'
      WHERE
      id=${todoId};`
